    ...
    <div className="wrapper">
      ...
      <section>{jobs.map((job, index) => {
        return <div key={index} className="job">
          <div  >  {job.url ? <a href={job.url}>{job.title}</a> : job.title}</div>
          <p>By {job.by} - {new Date(job.time * 1000).toLocaleString()}</p>
        </div>
      }
      )}</section>
      <button className="btn" onClick={() => { setPage(old => old + 1) }}> Load more jobs</button>
    </div>
  );
}
